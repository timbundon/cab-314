...
class ActionRealisations():
    ...
    def _collect_files(self, action, args):
        NETWORK_MANAGER.send_response(action, "200")
    
    def _broadcast_start(self, action, args):
        WINDOW_MANAGER.new_mode = WINDOW_MANAGER.Modes.TEACHERS_STREAM
        NETWORK_MANAGER.send_response(action, "200")
    
    def _broadcast_stop(self, action, args):
        WINDOW_MANAGER.new_mode = WINDOW_MANAGER.Modes.NONE
        NETWORK_MANAGER.send_response(action, "200")
    
    def _open_url(self, action, args):
        url = args.get("url")
        webbrowser.open(url)
        NETWORK_MANAGER.send_response(action, "200")
    ...
    def execute(self, action, args):
        ...
        if func:
            threading.Thread(target=func, args=(action, args), daemon=True).start()
        else:
            NETWORK_MANAGER.send_response(action, f"error: invalid action called {action}")
...
class NetworkManager():
    ...
    def send_response(self, action, result):
        self.socketio.emit("client_response", {"ip": IP_CONFIGURATION.CLIENT_IP, "action": action, "result": result})
    def connect(self):
    ...
    def setup_routes(self):
        ...
        def on_command(data):
            action = data["action"]
            args = data.get("args", {})
            ACTION_REALISATIONS.execute(action, args)
    def cleanup(self):
